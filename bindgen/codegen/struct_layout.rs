...
impl<'a> StructLayoutTracker<'a> {
    ...
    pub(crate) fn saw_field_with_layout(
        ...
        if self.ctx.options().generate_ensure_layout_attributes {
            return None;
        }
        ...
        if !self.ctx.options().remove_ghidra_autogenerated_fields {
    ...
    // pub(crate) fn field_index(&self) -> usize { self.field_index }
}
