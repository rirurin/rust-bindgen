...
impl FieldCodegen<'_> for FieldData {
        ...
        M: Extend<proc_macro2::TokenStream>,
    {
        ...
        if ctx.options().remove_ghidra_autogenerated_fields &&
            field_name == format!("field{}_0x{:x}", field_index, self.offset().unwrap() / 8)
        {
            if let syn::Type::Path(p) = &ty {
                let ty_name = p.path.get_ident().unwrap();
                match ty_name.to_string().as_str() {
                    "undefined" | "undefined2" | "undefined3" | "undefined4" |
                    "undefined5" | "undefined6" | "undefined7" | "undefined8" => return,
                    _ => ()
                };
            }
        }
        ...
        let mut field_decl = match visibility {
        ...
        if ctx.options().generate_ensure_layout_attributes {
            let offset_of_field = self.offset().unwrap() / 8;
            field_decl = quote! { #[field_offset(#offset_of_field)] #field_decl };
        }
        field.append_all(field_decl);
...
impl CodeGenerator for CompInfo {
    ...
    fn codegen(
        ...
        !ctx.options().remove_ghidra_autogenerated_fields {
        ...
        let mut tokens = if is_rust_union {
            ...
            if ctx.options().generate_ensure_layout_attributes { 
                if let Some(s) = &layout {
                    let struct_size = s.size;
                    quote! {
                        #( #attributes )*
                        #[ensure_layout(size = #struct_size)]
